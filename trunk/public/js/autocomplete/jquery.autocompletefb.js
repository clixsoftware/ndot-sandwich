...
jQuery.fn.autoCompletefb = function(options) 
{
	...
	var acfb = 
	{
		...
		removeFind : function(o){
			...
			document.getElementById('toidname').value = ids;
			return tmp.acfb;
		}
	}
	...
	$(settings.inputClass,tmp).result(function(e,d,f){
		...
		var v = '<li class="'+f+'"><span>'+d+'</span> <font style="color:red; font-weight:bold;" class="p"> X </font></li>';
		...
		var start = d.indexOf('>');
		var end = d.indexOf('</a>');
		var id = d.substring(start+1,end);
		if(document.getElementById('toidname').value == "")
		{
			document.getElementById('toidname').value = id;
		}else{	
			document.getElementById('toidname').value = document.getElementById('toidname').value+","+id;
		}
