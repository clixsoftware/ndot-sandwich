...
function deletemail(mid){
	...
	function movetoinbox(mid){
	
	var url = '/ajax/movetoinbox?mid='+mid;
	$.post(url,function(check){ 
...
//	alert(k);
  ...
  delete k;
	...
	// confirm dialog
		...
		jqDialog.confirm("Selected message will move to Inbox?",
			function() {
			moveto_inbox();
				},		// callback function for 'YES' button
			function() {
...
<a href="logout" style="color:#FFFFFF;font-size:14px;">Logout</a>-->
<div class="span-19a">

<div class="responsetwo"></div>
<!--top heading bar start
<div class="top_bar"><h1 class="heading">Archive Mail</h1></div>
-->
<!--topheadingbarfinish-->
...
<li><a  id="movetoinbox" href="javascript:;">Move to Inbox</a></li>
...
{
if(($inboxcontent->read_status==-1 || $inboxcontent->read_status!=$this->uid) && $inboxcontent->read_status!=3){
echo '<div class="inner_mail_unread  pl-5" id="'.$inboxcontent->sentmail_id.'">';
}else{
...
<div class="mail_con_out" onClick="showmail('<?php echo $inboxcontent->sentmail_id;?>','<?php echo $this->uid ?>')" onMouseDown="javascript:window.location='<?php echo $this->docroot;?>inbox/archive_reply/?mailid=<?php  echo $inboxcontent->sentmail_id;?>'">
...
<!--<label class="deleteout" onclick="deletemailone('<?php //echo $inboxcontent->sentmail_id;?>');">Delete
</label>--><?php echo '</div>';?>
<?php }?>
...
<li><a  id="movetoinboxone" href="javascript:;">Move to Inbox</a></li>
</ul>
</div>
